...
def get_custom_error_messages():
    return {
        "_comment": "You did not provide any field named <field_name> in your data dictionary",
...
def retrieve_date_format(field_rules):
...
class Validation:
    ...
    def __init__(self):
        self.error_message_templates = get_error_message_templates()
    ...
    def validate(self, data, rules, custom_messages=None):
        ...
        for field_name in rules:
            ...
            # fetch the rule (value of dictionary element) from "rules" dictionary for the current rule name (
            # dictionary key) and split it to get a list
    ...
    def validate_date_fields(self, data, field_name, field_rules):
        ...
        errs = []
        date_format = retrieve_date_format(field_rules)
        try:
    ...
    def validate_email_fields(self, data, field_name):
        ...
        # in case the result not match or there was a key error
        if not result:
    ...
    def validate_regex_fields(self, data, field_name, rule):
        ...
        # in case the result did not match or there was a key error
        if not result:
    ...
    def validate_confirmed_fields(self, data, field_name):
        """if the field under validation is a password, a matching password_confirmation field must be present in the
        data, returns a list of error messages"""
    ...
    def validate_size_fields(self, data, field_name, rule):
        """Used for validating fields for a maximum number of characters in a string value, returns a list of error
        messages"""
    ...
    def validate_different_fields(self, data, field_name, rule):
        """
        Used for validating fields whose value should be different from the value of some other field,
    ...
    def is_valid(self, data, rules):
        """Validates the data according to the rules, returns True if the data is valid, and False if the data is
        invalid"""
